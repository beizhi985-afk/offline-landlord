...
- 首张 Reverse：以随机确定的基准首位玩家为起点。2 人局等同 Skip，由另一名玩家先行动；3/4 人局先将方向翻转为逆时针，再由基准玩家逆时针方向的下一位（即基准玩家之前的一位）先行动。
- 罚摸与 Catch UNO：每次只摸一张真实可用牌；摸牌堆为空时，保留弃牌堆顶牌并洗回其余弃牌后继续摸。若除顶牌外已无任何牌，则在已摸到全部真实可用牌后停止，不复制、不虚构牌，也不会造成死锁。
- 以下规则保持不变：经典 108 张牌；2～4 人；禁止 +2/+4 叠加、Jump-In、7-0；Wild Draw Four 的颜色合法性；UNO 声明与 Catch UNO；Quick 与 Points 500 计分。
